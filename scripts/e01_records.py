...
def verify_frozen_outputs(plan_path: str | Path, manifest_path: str | Path, reference_path: str | Path) -> dict[str, str]:
    """Refuse execution unless generated bytes match the frozen execution plan."""
    plan = json.loads(Path(plan_path).read_text(encoding="utf-8"))
    expected = plan["artifacts"]
    actual = {
        "episode_manifest_sha256": _sha256(Path(manifest_path)),
        "reference_answers_sha256": _sha256(Path(reference_path)),
    }
    mismatches = {
    ...
    if mismatches:
        raise ValueError(f"frozen E01 artifact hash mismatch: {mismatches}")
    return actual
...
def main() -> int:
    ...
    if config.get("artifacts", {}).get("generator_revision") not in (None, GENERATOR_REVISION):
        raise ValueError("execution plan generator revision does not match records-v1")
    if config.get("artifacts", {}).get("scorer_revision") not in (None, SCORER_REVISION):
        raise ValueError("execution plan scorer revision does not match e01-records-scorer-v1")
